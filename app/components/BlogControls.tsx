...
export const BlogControls = ({
  ...
  <div className="rounded-3xl border border-white/70 bg-white/80 p-5 shadow-[0_20px_60px_-45px_rgba(31,41,55,0.5)] backdrop-blur">
    ...
    <input
      ...
      className="mt-2 w-full rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-700 outline-none transition focus:border-[var(--accent)] focus:ring-2 focus:ring-orange-100"
      placeholder="nextjs, aws, markdown"
    />
    <button
      type="button"
      onClick={onSave}
    ...
    <button
      type="button"
      onClick={onNewPost}
      className="mt-3 w-full rounded-full border border-zinc-200 bg-white px-4 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-800"
    >
      New post
    </button>
  </div>
